...
function parseTestSuites(content, cwd) {
    ...
    const allTests = (discoveryResult.tests || [])
        ...
        .filter(line => line)
        .map(line => line);
    const suites = Array.from((0, collections_1.groupBy)(allTests, t => t.modulePath).entries())
    ...
    const aggregatedErrors = Array.from((0, collections_1.groupBy)((discoveryResult.errors || []), e => e.file).entries())
        .map(([file, messages]) => ({
...
function toSuites(suites) {
    ...
    return Array.from((0, collections_1.groupBy)(suites.map(test => splitTest(test)), group => group.idHead).entries())
