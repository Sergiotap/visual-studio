...
function registerTestAdapters(wf, extension, loggerFactory) {
    ...
    const unittestRunner = new unittestTestRunner_1.UnittestTestRunner((0, idGenerator_1.nextId)(), unittestLogger);
    const pytestLogger = loggerFactory('pytest', wf);
    const pytestRunner = new pytestTestRunner_1.PytestTestRunner((0, idGenerator_1.nextId)(), pytestLogger);
    const testplanLogger = loggerFactory('testplan', wf);
    const testplanRunner = new testplanTestRunner_1.TestplanTestRunner((0, idGenerator_1.nextId)(), pytestLogger);
